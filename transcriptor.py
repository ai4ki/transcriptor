...
def transcribe(prompt):
    ...
    audio_file = open(audiofile.name, "rb")
    ...
    try:
        ...
        df_transcript, text_transcript = speaker_diarization(response)
...
def speaker_diarization(response, num_speakers=0):

    if audiofile.name.endswith(".wav"):
        wave_file = audiofile.name
    else:
        file_name, _ = os.path.splitext(audiofile.name)
        wave_file = file_name + ".wav"
        os.system(f'ffmpeg -i "{audiofile.name}" -ar 16000 -ac 1 -c:a pcm_s16le "{wave_file}"')
        ...
        if audiofile is not None:
            if save_uploaded_file(audiofile):
                ...
                with st.spinner(f"Transkription von {audiofile.name} läuft..."):
                    df_transcription, transcription = transcribe("")
